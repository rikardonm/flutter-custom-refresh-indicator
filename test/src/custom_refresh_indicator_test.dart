...
Widget buildWithoutIndicator(BuildContext context, Widget child, IndicatorController controller) => child;
...
class DefaultList extends StatelessWidget {
  ...
  const DefaultList({super.key, required this.itemsCount, this.reverse = false, this.controller, this.physics = const AlwaysScrollableScrollPhysics()});
  ...
  Widget build(BuildContext context) {
    return ListView.builder(
      ...
      reverse: reverse,
      itemBuilder: (context, index) => SizedBox(height: 200, child: Text((index + 1).toString())),
    );
  }
}
...
void main() {
  ...
  testWidgets('CustomRefreshIndicator - default behaviour', (WidgetTester tester) async {
    await tester.pumpWidget(
      MaterialApp(
        home: CustomRefreshIndicator(builder: buildWithoutIndicator, onRefresh: (t) => fakeRefresh.instantRefresh(), child: const DefaultList(itemsCount: 6)),
      ),
    );
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: CustomRefreshIndicator(
          ...
          onRefresh: (t) => fakeRefresh.instantRefresh(),
          child: const SingleChildScrollView(
            scrollDirection: Axis.horizontal,
            child: SizedBox(width: 600.0, child: DefaultList(itemsCount: 6)),
          ),
        ),
      ),
    );
  ...
  testWidgets('CustomRefreshIndicator - reverse', (WidgetTester tester) async {
    await tester.pumpWidget(
      MaterialApp(
        home: CustomRefreshIndicator(builder: buildWithoutIndicator, onRefresh: (t) => fakeRefresh.instantRefresh(), child: const DefaultList(itemsCount: 1, reverse: true)),
      ),
    );
  ...
  testWidgets('CustomRefreshIndicator - top - position', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: CustomRefreshIndicator(controller: controller, builder: buildWithoutIndicator, onRefresh: fakeRefresh.refresh, child: const DefaultList(itemsCount: 1)),
      ),
    );
  ...
  testWidgets('CustomRefreshIndicator - no movement', (WidgetTester tester) async {
    await tester.pumpWidget(
      MaterialApp(
        home: CustomRefreshIndicator(onRefresh: (t) => fakeRefresh.instantRefresh(), builder: buildWithoutIndicator, child: const DefaultList(itemsCount: 1)),
      ),
    );
  ...
  testWidgets('CustomRefreshIndicator - not enough', (WidgetTester tester) async {
    await tester.pumpWidget(
      MaterialApp(
        home: CustomRefreshIndicator(builder: buildWithoutIndicator, offsetToArmed: 200.0, onRefresh: (t) => fakeRefresh.instantRefresh(), child: const DefaultList(itemsCount: 1)),
      ),
    );
  ...
  testWidgets('CustomRefreshIndicator - refresh (programmatically) - slow', (WidgetTester tester) async {
    await tester.pumpWidget(
      MaterialApp(
        home: CustomRefreshIndicator(builder: buildWithoutIndicator, onRefresh: fakeRefresh.refresh, child: const DefaultList(itemsCount: 1)),
      ),
    );
  ...
  testWidgets('CustomRefreshIndicator - refresh (programmatically) - fast', (WidgetTester tester) async {
    await tester.pumpWidget(
      MaterialApp(
        home: CustomRefreshIndicator(builder: buildWithoutIndicator, onRefresh: (t) => fakeRefresh.instantRefresh(), child: const DefaultList(itemsCount: 1)),
      ),
    );

    bool completed = false;

    tester.state<CustomRefreshIndicatorState>(find.byType(CustomRefreshIndicator)).refresh().then<void>((_) => completed = true);
  ...
  testWidgets('CustomRefreshIndicator - refresh (programmatically) - fast - twice', (WidgetTester tester) async {
    await tester.pumpWidget(
      MaterialApp(
        home: CustomRefreshIndicator(builder: buildWithoutIndicator, onRefresh: (t) => fakeRefresh.instantRefresh(), child: const DefaultList(itemsCount: 1)),
      ),
    ...
    expect(() => tester.state<CustomRefreshIndicatorState>(find.byType(CustomRefreshIndicator)).refresh(), throwsA(isA<StateError>()));
  ...
  testWidgets('CustomRefreshIndicator - hide (programmatically) - when not shown', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: CustomRefreshIndicator(controller: indicatorController, builder: buildWithoutIndicator, onRefresh: fakeRefresh.refresh, child: const DefaultList(itemsCount: 1)),
      ),
    );

    final state = tester.state<CustomRefreshIndicatorState>(find.byType(CustomRefreshIndicator));
      ...
      await tester.pumpWidget(
        MaterialApp(
          home: CustomRefreshIndicator(
            ...
            builder: buildWithoutIndicator,
            durations: const RefreshIndicatorDurations(settleDuration: Duration(milliseconds: 150)),
            onRefresh: (t) => fakeRefresh.instantRefresh(),
  ...
  testWidgets('CustomRefreshIndicator - trigger mode - anywhere - should be shown when dragging from non-zero scroll position', (WidgetTester tester) async {
    final indicatorController = IndicatorController();
  ...
  testWidgets('CustomRefreshIndicator - reverse - trigger mode - anywhere - should be shown when dragging from non-zero scroll position', (WidgetTester tester) async {
    final indicatorController = IndicatorController();
    final ScrollController scrollController = ScrollController();
    await tester.pumpWidget(
      MaterialApp(
        home: CustomRefreshIndicator(
          ...
          triggerMode: IndicatorTriggerMode.anywhere,
          onRefresh: fakeRefresh.refresh,
          child: DefaultList(controller: scrollController, itemsCount: 6, reverse: true),
        ),
      ),
    );
  ...
  testWidgets('CustomRefreshIndicator - trigger mode - anywhere - should not be shown when overscroll occurs due to inertia', (WidgetTester tester) async {
    final indicatorController = IndicatorController();
    final ScrollController scrollController = ScrollController();
    await tester.pumpWidget(
      MaterialApp(
  ...
  testWidgets('CustomRefreshIndicator - trigger mode - onEdge - should not be shown when dragging from non-zero scroll position', (WidgetTester tester) async {
    final indicatorController = IndicatorController();
    await tester.pumpWidget(
      MaterialApp(
        home: CustomRefreshIndicator(builder: buildWithoutIndicator, onRefresh: fakeRefresh.refresh, child: const DefaultList(itemsCount: 6)),
      ),
    );
  ...
  testWidgets('CustomRefreshIndicator - reverse - trigger mode - onEdge - should be shown when dragging from non-zero scroll position', (WidgetTester tester) async {
    final indicatorController = IndicatorController();
    await tester.pumpWidget(
      MaterialApp(
        home: CustomRefreshIndicator(
          ...
          controller: indicatorController,
          onRefresh: fakeRefresh.refresh,
          child: const DefaultList(itemsCount: 6, reverse: true),
        ),
      ),
    );
  ...
  testWidgets('CustomRefreshIndicator - ScrollController.jumpTo - should not trigger the refresh indicator', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: CustomRefreshIndicator(
          builder: buildWithoutIndicator,
          onRefresh: (t) => fakeRefresh.instantRefresh(),
          child: const DefaultList(itemsCount: 4, reverse: true, physics: BouncingScrollPhysics()),
        ),
      ),
    );
          ...
          child: Builder(
            builder: (BuildContext context) {
              return NotificationListener<ScrollNotification>(
                onNotification: (ScrollNotification notification) {
                  if (notification is OverscrollNotification && lastNotification is! OverscrollNotification) {
                    final OverscrollIndicatorNotification confirmationNotification = OverscrollIndicatorNotification(leading: false);
                    confirmationNotification.dispatch(context);
                    glowAccepted = confirmationNotification.accepted;
                  }
                  lastNotification = notification;
  ...
  testWidgets('CustomRefreshIndicator - trigger - both edges  - should be shown when dragging from the start or end edge', (WidgetTester tester) async {
    final indicatorController = IndicatorController();
    await tester.pumpWidget(
      MaterialApp(
        home: CustomRefreshIndicator(
  ...
  testWidgets('CustomRefreshIndicator - trigger - end edge  - should be shown only when dragging from the end edge', (WidgetTester tester) async {
    final indicatorController = IndicatorController();
    await tester.pumpWidget(
      MaterialApp(
        home: CustomRefreshIndicator(
  ...
  testWidgets('CustomRefreshIndicator - trigger - start edge  - should be shown only when dragging from the end edge', (WidgetTester tester) async {
    final indicatorController = IndicatorController();
    await tester.pumpWidget(
      MaterialApp(
        home: CustomRefreshIndicator(
  ...
  testWidgets('CustomRefreshIndicator - onStateChanged', (WidgetTester tester) async {
    ...
    expect(changes, [
      const IndicatorStateChange(IndicatorState.idle, IndicatorState.dragging),
      const IndicatorStateChange(IndicatorState.dragging, IndicatorState.armed),
      const IndicatorStateChange(IndicatorState.armed, IndicatorState.settling),
      const IndicatorStateChange(IndicatorState.settling, IndicatorState.loading),
      const IndicatorStateChange(IndicatorState.loading, IndicatorState.finalizing),
      const IndicatorStateChange(IndicatorState.finalizing, IndicatorState.idle),
    ]);
  ...
  testWidgets('CustomRefreshIndicator - onStateChanged - with completed state', (WidgetTester tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: CustomRefreshIndicator(
          ...
          builder: buildWithoutIndicator,
          onRefresh: (t) => fakeRefresh.instantRefresh(),
          durations: const RefreshIndicatorDurations(completeDuration: Duration(milliseconds: 300)),
          child: const DefaultList(itemsCount: 1),
        ),
      ),
    );
    ...
    expect(changes, [
      ...
      const IndicatorStateChange(IndicatorState.armed, IndicatorState.settling),
      const IndicatorStateChange(IndicatorState.settling, IndicatorState.loading),
      const IndicatorStateChange(IndicatorState.loading, IndicatorState.complete),
      const IndicatorStateChange(IndicatorState.complete, IndicatorState.finalizing),
      const IndicatorStateChange(IndicatorState.finalizing, IndicatorState.idle),
    ]);
  ...
  testWidgets('CustomRefreshIndicator - BouncingPhysics - start from scroll update notification', (WidgetTester tester) async {
    final indicatorController = IndicatorController();
  ...
  testWidgets('CustomRefreshIndicator - autoRebuild - true - rebuilds the builder function with each change', (WidgetTester tester) async {
  ...
  testWidgets('CustomRefreshIndicator - autoRebuild - false - invokes the builder function only for state changes', (WidgetTester tester) async {
    int indicatorChangesCount = 0;
    int rebuildsCount = 0;
    ...
    expect(states, equals([IndicatorState.dragging, IndicatorState.armed, IndicatorState.settling, IndicatorState.loading, IndicatorState.finalizing, IndicatorState.idle]));
